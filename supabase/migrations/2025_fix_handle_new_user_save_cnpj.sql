-- Migração: Atualizar função handle_new_user para salvar CNPJ em public.clinics
-- Objetivo: garantir que o CNPJ fornecido nos metadados do usuário seja persistido na coluna pública clinics.cnpj
...
BEGIN
  ...
  IF COALESCE(NEW.raw_user_meta_data ->> 'role', 'patient') = 'clinic' THEN
    ...
    -- Criar registro na tabela clinics (sem CNPJ aqui)
