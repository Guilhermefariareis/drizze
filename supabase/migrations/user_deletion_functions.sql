...
--
-- These functions operate on public tables referencing public.profiles(id) or auth.users(id).
-- delete_user_cascade removes children first, then the public.profiles row.
-- It DOES NOT delete from auth.users; use Admin API for that.
...
comment on function public.delete_user_cascade(uuid) is 'Delete user-related public rows referencing profiles/users, then delete public.profiles; DOES NOT delete